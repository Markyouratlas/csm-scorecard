      ...
      if (mode === 'signup') {
        ...
        if (data.user) {
          ...
          const { error: profileError } = await supabase.from('profiles').insert({
            ...
            team,
            role_type: roleType,
            // Auto-promote on signup: first-user (executive seed) OR any role
            // whose role_type ends in '_lead' (e.g. forward_deployed_engineer_lead).
            // Other team leads (CSM team lead, etc.) are still toggled manually
            // by an exec via Manager view; for those, no lead-specific role exists.
            is_team_lead: isFirstUser || roleType.endsWith('_lead'),
