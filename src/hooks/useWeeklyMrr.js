...
export function useWeeklyMrr({ monthlyAnchors = [], liveMrr = null, weeks = 8 } = {}) {
  ...
  const weekKeys = recentWeekKeys(weeks) // oldest → newest, current week last
  ...
  const anchors = monthlyAnchors
    ...
    .sort((x, y) => x.t - y.t)
  if (liveMrr != null && weekKeys.length) {
    const nowT = toUTCms(weekKeys[weekKeys.length - 1])
  ...
  const series = weekKeys.map((wk, i) => {
    if (ov[wk] != null) return { week: `W${i + 1}`, weekKey: wk, mrr: Math.round(ov[wk]), source: 'manual' }
    const v = interpolate(anchors, toUTCms(wk))
    return { week: `W${i + 1}`, weekKey: wk, mrr: v != null ? Math.round(v) : null, source: v != null ? 'interpolated' : 'none' }
  })
